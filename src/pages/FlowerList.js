...
const FlowerList = () => {
  ...
  return (
    <div className="max-w-[1220px] mx-auto">
      <div className="text-gray-400 focus-within:text-gray-600 max-w-[1220px] w-[600px] mx-auto h-[70px] relative mt-10  ">
        <input
        ...
        <div className="absolute right-3 top-7 ">
          <BsSearch className=" w-5 h-5  text-red-500" />
        </div>
      </div>

      <section className="flex flex-wrap  gap-4 mt-[90px]  px-4 ">
        {data?.map((flower) => (
          <FlowerCard
            key={flower.id}
            name={flower.name}
